...
static void choose_race()
{
  ...
  l = 2;
  m = 21;
  clear_from (Char_window, 20);
  put_buffer(Char_window, "Choose a race (? for Help):", 20, 2);
  do
    {
      (void) sprintf(tmp_str, "%c) %s", k+'a', race[j].trace);
      put_buffer(Char_window, tmp_str, m, l);
      k++;
      l += 15;
      if (l > 70)
	{
  ...
  r_ptr = &race[j];
  p_ptr->misc.prace  = j;
  put_buffer(Char_window, r_ptr->trace, 3, 15);
}
...
static void print_history()
{
  ...
  put_buffer(Char_window, "Character Background", 14, 27);
  for (i = 0; i < 4; i++)
    prt(Char_window, py.misc.history[i], i+15, 10);
}
...
static void get_sex()
{
  ...
  put_character();
  clear_from(Char_window, 19);
  put_buffer(Char_window, "Choose a sex (? for Help):", 20, 2);
  put_buffer(Char_window, "m) Male       f) Female", 21, 2);
  ...
  do
	...
	{
	  py.misc.male = FALSE;
	  put_buffer(Char_window, "Female", 4, 15);
	  exit_flag = TRUE;
	}
      else if (c == 'm' || c == 'M')
	{
	  py.misc.male = TRUE;
	  put_buffer(Char_window, "Male", 4, 15);
	  exit_flag = TRUE;
	}
      else if (c == '?')
	helpfile (MORIA_WELCOME);
...
static void get_class()
{
  ...
  mask = 0x1;
  clear_from (Char_window, 19);
  put_buffer(Char_window, "Choose a class (? for Help):", 20, 2);
  do
    {
  ...
  do
      ...
      if ((j < k) && (j >= 0))
	{
	  ...
	  exit_flag = TRUE;
	  clear_from (Char_window, 19);
	  put_buffer(Char_window, c_ptr->title, 5, 15);
