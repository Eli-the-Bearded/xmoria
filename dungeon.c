	      ...
	      else
		{
		  ...
		  if (command_count > 0)
			  ...
			  || (!rogue_like_commands && command == '#'))
			{
			  ...
			  while (TRUE)
			      ...
			      if (command == DELETE || command == CTRL('H'))
				{
				  ...
				  put_buffer(Status_window, tmp, 0, 14);
				}
			  ...
			  if (command == ' ')
			    {
			      put_buffer(Status_window, "Command:", 0, 20);
			      command = inkey(TRUE);
			    }
			}
		      ...
		      if (command == '^')
			{
			  if (command_count > 0)
			    prt_state();
			  if (get_com(Status_window, "Control-", &command))
			    {
...
char com_val;
{
  ...
  switch(com_val)
    {
    case 'Q':	/* (Q)uit		(^K)ill */
      if (get_check(Status_window, "Do you really want to quit?"))
	{
	  new_level_flag = TRUE;
	  death = TRUE;
      ...
      if (i > 1)
	{
	  ...
	  while (i > 0)
	    {
	      i--;
	      prt(Inven_window, old_msg[j], i, 0);
	      if (j == 0)
		j = MAX_SAVE_MSG-1;
	      else
		j--;
	    }
	  prt(Inven_window, "Press any key to continue", i, 0);
	  inkeyr(Inven_window);
      ...
      else
	{
	  ...
	  put_buffer(Status_window, ">", mfontheight, 0);
	  prt(Status_window, old_msg[j], 1, 1);
	}
      free_turn_flag = TRUE;
      break;
...
*/
      ...
      if (wizard)
	{
	  ...
	  switch(com_val)
		...
		{
		  msg_print("Go to which level (0-99) ? ");
		  i = -1;
	    ...
	    default:
	      if (rogue_like_commands)
		msg_print("Type '?' or '\\' for help.");
	      else
		msg_print("Type '?' or ^H for help.");
	    }
	}
      else
	{
...
static void examine_book()
{
  ...
  if (!find_range(TV_MAGIC_BOOK, TV_PRAYER_BOOK, &i, &k))
      ...
      else
	{
	  ...
	  Spell_win = CreateDWin(400, 10 * mfontheight, 80, 55,
				   Main, mfgpix, mbgpix);
