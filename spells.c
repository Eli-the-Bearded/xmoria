...
extern int set_null();
extern int set_lightning_destroy();
extern int set_acid_destroy();
extern int set_frost_destroy();
extern int set_fire_destroy();
...
int detect_sdoor()
{
  ...
  detect = FALSE;
  for (i = (panel_row_min > 20) ? panel_row_min - 20 : 0;
       i <= panel_row_max + 20; i++)
    for (j = (panel_col_min > 20) ? panel_col_min - 15 : 0;
	 j <= panel_col_max + 20; j++)
      {
	c_ptr = &cave[i][j];
	if (c_ptr->tptr != 0)
	  /* Secret doors  */
...
int detect_invisible()
{
  ...
  for (i = mfptr - 1; i >= MIN_MONIX; i--)
	  ...
	  (CM_INVISIBLE & c_list[m_ptr->mptr].cmove))
	{
	  ...
	  PrintMapChar( (char)c_list[m_ptr->mptr].cchar,
                        ((int)m_ptr->fx - panel_col_min),
                        ((int)m_ptr->fy - panel_row_min));
	  flag = TRUE;
	}
    }
  if (flag)
...
int ident_spell()
{
  ...
  ident = FALSE;
  if (get_item(&item_val, "Item you wish identified?", 0, inven_ctr - 1))
    {
      ident = TRUE;
      identify(&item_val);
...
int genocide()
{
  ...
  killed = FALSE;
  if (get_com(Status_window,
	      "Which type of creature do you wish exterminated?", &typ))
    for (i = mfptr - 1; i >= MIN_MONIX; i--)
      {
	m_ptr = &m_list[i];
	r_ptr = &c_list[m_ptr->mptr];
...
int detect_evil()
{
  ...
  for (i = mfptr - 1; i >= MIN_MONIX; i--)
	  ...
	  (CD_EVIL & c_list[m_ptr->mptr].cdefense))
	{
	  m_ptr->ml = TRUE;
	  /* works correctly even if hallucinating */
	  PrintMapChar((char)c_list[m_ptr->mptr].cchar, (int)m_ptr->fy,
		(int)m_ptr->fx);
	  flag = TRUE;
	}
    }
