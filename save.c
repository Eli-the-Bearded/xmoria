  ...
  while (!_save_char(savefile))
    {
	  ...
	  || get_check(Status_window, "File exists. Delete old savefile?") == 0
	  || (i = unlink(savefile)) < 0)
	{
	  if (i < 0)
	    {
	  ...
	  msg_print("New Savefile [ESC to give up]:");
	  if (!get_string(Status_window, temp, 0, 31, 45))
	    return FALSE;
	  if (temp[0])
...
char *fnam;
{
  ...
  if (fd < 0 && access(fnam, 0) >= 0 &&
      (from_savefile ||
       (wizard && get_check(Status_window, "Can't make new savefile. Overwrite old?"))))
    {
...
int *generate;
{
  ...
  *generate = TRUE;
  ...
  if (turn >= 0)
    puts("IMPOSSIBLE! Attempt to restore while still alive!");
  ...
  /* Hence first try to read without doing a chmod. */
  ...
  else
    {
      ...
      (void) close(fd);
      /* GCC for atari st defines atarist */
      fileptr = (FILE *)fopen(savefile, "r");
      if (fileptr == NULL)
	goto error;

      puts("Restoring Memory...");
      put_qio();
	...
	{
	  puts("Sorry. This savefile is from a different version of umoria.");
	  goto error;
	}
	...
	find_ignore_doors = FALSE;
	  ...
	  && get_check(Status_window, "Resurrect a dead character?"))
	l &= ~0x80000000L;
      if ((l & 0x80000000L) == 0)
	{
	  m_ptr = &py.misc;
      ...
      if ((c = getc(fileptr)) == EOF || (l & 0x80000000L))
	{
	  if ((l & 0x80000000L) == 0)
		...
		goto error;
	      puts("Attempting a resurrection!");
	      if (py.misc.chp < 0)
		{
		  py.misc.chp =	 0;
		  py.misc.chp_frac = 0;
	    ...
	    {
	      puts("Restoring Memory of a departed spirit...");
	      turn = -1;
	    }
	  put_qio();
	...
	(void) close(fd);
      ...
      if (!ok)
	puts("Error during reading of file.");
      else if (turn >= 0 && !_new_log())
	puts("Can't log player in the log file.");
      else
	{
	  ...
	  if (panic_save == 1)
	    {
	      puts("This game is from a panic save.  Score will not be added to scoreboard.");
	    }
	  ...
	  if (noscore)
	    puts("This save file cannot be used to get on the score board.");
  ...
  turn = -1;
  log_index = -1;
  puts("Please try again without that savefile.");
  signals();
#ifdef MAC
  *exit_flag = TRUE;
#else
